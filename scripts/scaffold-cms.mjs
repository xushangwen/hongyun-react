...
const componentSchemas = {
  ...
  'content.equipment-grid': {
    ...
    attributes: {
      ...
      visible,
      title: { type: 'string' },
      equipmentKeys: structuredJson('string-list', { required: true }),
...
const contentTypes = {
  ...
  'technical-dataset': {
    ...
    attributes: {
      title: { type: 'string', required: true, ...localized },
      kind: { type: 'enumeration', enum: ['spec-table', 'experiment-table', 'chart-data', 'eds-data'], required: true, ...nonLocalized },
      schemaVersion: { type: 'integer', min: 1, required: true, ...nonLocalized },
