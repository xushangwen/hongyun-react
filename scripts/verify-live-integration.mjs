...
function collectMedia(value, urls = new Set()) {
  if (Array.isArray(value)) value.forEach((item) => collectMedia(item, urls))
    ...
    if (typeof value.url === 'string' && /\/uploads\//.test(value.url)) urls.add(value.url)
...
for (const path of productRoutes) {
  ...
  if (!detail.title || !detail.sections?.length) throw new Error(`${path} 缺少可渲染详情`)
...
const mediaUrls = collectMedia([categories, productDetails, solutionDetails, news])
const failures = []
